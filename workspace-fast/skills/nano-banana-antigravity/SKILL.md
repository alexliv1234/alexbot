...
description: Generate or edit images via Gemini with Google AI API key
homepage: https://ai.google.dev
metadata: {"openclaw":{"emoji":"🍌","requires":{"bins":["uv"]}}}
---

# Gemini Image Generation (Google AI API Key)

Generate images using Google Gemini's image generation capabilities.

**Uses direct API key** (no OAuth needed!) - faster and more reliable.

## Generate Image (Recommended - API Key)

**Quick generation:**
```bash
uv run {baseDir}/scripts/generate_image_apikey.py --prompt "your image description" --filename "output.png"
```

**For WhatsApp HD:**
```bash
uv run {baseDir}/scripts/generate_image_apikey.py \
...
## Legacy: OAuth Method (rate-limited)
