...
# Generate with temp PNG name (use API key method - faster and no rate limits)
uv run "$SCRIPT_DIR/generate_image_apikey.py" "${ARGS[@]}" --filename "$TEMP_PNG"
...
if [[ "$FILENAME" == *.jpg ]] || [[ "$FILENAME" == *.jpeg ]]; then
    ...
    # Use magick (v7) or convert (v6) - whichever is available
    CONVERT_CMD="magick"
    if ! command -v magick &>/dev/null; then
        CONVERT_CMD="convert"
    fi
    
    $CONVERT_CMD "$TEMP_PNG" \
        -interlace Plane \
        -quality "$QUALITY" \
        "$FILENAME"
