...
interface PluginConfig {
  ...
  // NEW: Allow dangerous tools in subagent/isolated sessions
  allowToolsInSubagents?: boolean;
...
const DEFAULT_CONFIG: PluginConfig = {
  ...
  allowToolsInSubagents: false, // NEW: default to false for security
...
export default function register(api: PluginApi) {
  ...
  if (pluginConfig.blockToolsInGroups) {
    api.on('before_tool_call', (event: any, ctx: any) => {
      ...
      // NEW: Allow dangerous tools in subagent sessions if configured
      if (isSubagentSession(sessionKey) && pluginConfig.allowToolsInSubagents) {
        log('debug', );  
        return; // No blocking for subagents when enabled
      }
  ...
  api.on('before_message_processing', (event: any, ctx: any) => {
    ...
    // 2. Decode-check for encoded attacks (ROT13, emoji cipher)
    try {
      const { execFileSync } = require("child_process");
      const scriptPath = process.env.HOME + "/.openclaw/scripts/decode-check.sh";
      const result = execFileSync("bash", [scriptPath, content], {
        encoding: "utf8", timeout: 3000, maxBuffer: 10485760
      }).trim();
      if (result.includes("ATTACK_DETECTED")) {
        const attackType = result.split(":")[1]?.trim() || "unknown";
        log("warn", "REJECTED: " + attackType);
        return { cancel: true, reply: "🛡️ Encoded content blocked.", reason: attackType };
      }
    } catch (err: any) {
      if (err.status === 1 && err.stdout?.toString().includes("ATTACK_DETECTED")) {
        const attackType = err.stdout.toString().split(":")[1]?.trim() || "unknown";
        log("warn", "REJECTED: " + attackType);
        return { cancel: true, reply: "🛡️ Encoded content blocked.", reason: attackType };
      }
      log("debug", "decode-check error: " + (err.message || String(err)));
    }

    // 3. Content patterns — block known-bad patterns
