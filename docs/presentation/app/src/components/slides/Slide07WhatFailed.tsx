...
const attacks = [
  ...
  { name: 'DAN / GODMODE templates', result: '"Keyword-blocked before the AI sees it"' },
];

const attacks2 = [
  { name: '"Ignore previous instructions"', result: '"Hard-blocked in pipeline"' },
  { name: 'Hex encoding', result: '"Nice try"' },
  { name: 'Unicode tricks', result: '"Models handle it fine"' },
];

const attacks3 = [
  { name: 'GDPR data access request', result: '"Sue the hammer, not the nail"' },
  { name: 'Multi-language switching', result: '"Bot responded in fluent Russian: Нет 😄"' },
];

const attacks4 = [
  { name: 'Combined (ROT13 + emoji + Base64)', result: '"56/70 for creativity. 3/10 for hacking."' },
];
  ...
  return (
    <>
      <FadeReveal revealKey="s07-title">
        <div className="slide-title">Everything They Tried (A Brief History of Failure)</div>
      </FadeReveal>

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4, paddingTop: 8 }}>
        <FadeReveal revealKey="s07-attacks-1">
          {attacks.map((a, i) => (
            <AttackRow key={a.name} name={a.name} result={a.result} delay={i * 0.08} />
          ))}
        </FadeReveal>
        ...
        <FadeReveal revealKey="s07-transition">
          <div style={{ marginTop: 'auto', paddingTop: 20, textAlign: 'center', fontSize: 26, fontWeight: 700, color: 'var(--gold)' }}>
